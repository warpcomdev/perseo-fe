...
function doIt(action, event, callback) {
    try {
        ...
        if (options.json) {
            ...
            // Must be a Buffer, String or ReadStream.
            // NOTE: stringifying a string would return a quoted string,
            // which is most probably NOT what an user of template wants
            // (i.e. JSON.stringify("hello") === '"hello"', which would be sent
            // over the wire including the quotes, with no way to remove them).
            // If the user actually wants the quotes, he can add them to the
            // template, or use the `json` property.
